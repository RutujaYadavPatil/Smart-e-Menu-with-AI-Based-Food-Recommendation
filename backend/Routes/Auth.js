...
const jwtSecret = "IamVishalPatil"
// var foodItems= require('../index').foodData;
// require("../index")
//Creating a user and storing data to MongoDB Atlas, No Login Requiered
router.post('/createuser', [
    body('email').isEmail(),
    body('password').isLength({ min:8 }).withMessage('Password must be at least 8 characters long').matches(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).*$/)
...
router.post('/getusername',async (req, res) => {
   try {
        ...
        const user = await User.findOne({ email: userEmail }).select("name"); // Find the user by email and select only the name field
        if (!user) {
